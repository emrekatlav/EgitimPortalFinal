...
getExecuteImgComposition function. This is where the destiFrontend canvas
...
(function($) {
    ...
    function composeImages(canvasOrSvgSources, destiFrontendCanvas) {
        var validCanvasOrSvgSources = canvasOrSvgSources.filter(isValidSource);
        pixelRatio = getPixelRatio(destiFrontendCanvas.getContext('2d'));
        ...
        var lastPromise = Promise.all(allImgCompositionPromises).then(getExecuteImgComposition(destiFrontendCanvas), failureCallback);
        return lastPromise;
    }
    ...
    function getExecuteImgComposition(destiFrontendCanvas) {
        return function executeImgComposition(tempImgs) {
            var compositionResult = copyImgsToCanvas(tempImgs, destiFrontendCanvas);
    ...
    function adaptDestSizeToZoom(destiFrontendCanvas, sources) {
        ...
        if (sources.find(containsSVGs) !== undefined) {
            if (pixelRatio < 1) {
                destiFrontendCanvas.width = destiFrontendCanvas.width * pixelRatio;
                destiFrontendCanvas.height = destiFrontendCanvas.height * pixelRatio;
            }
        }
    }

    function prepareImagesToBeComposed(sources, destiFrontend) {
        ...
        if (sources.length === 0) {
            ...
            if ((maxX - minX <= 0) || (maxY - minY <= 0)) {
                ...
                destiFrontend.width = Math.round(maxX - minX);
                destiFrontend.height = Math.round(maxY - minY);
                ...
                adaptDestSizeToZoom(destiFrontend, sources);
            }
        }
        return result;
    }

    function copyImgsToCanvas(sources, destiFrontend) {
        var prepareImagesResult = prepareImagesToBeComposed(sources, destiFrontend);
        if (prepareImagesResult === SUCCESSFULIMAGEPREPARATION) {
            var destiFrontendCtx = destiFrontend.getContext('2d');

            for (var i = 0; i < sources.length; i++) {
                if (sources[i].successfullyLoaded === true) {
                    destiFrontendCtx.drawImage(sources[i], sources[i].xCompOffset * pixelRatio, sources[i].yCompOffset * pixelRatio);
                }
            }
        }
